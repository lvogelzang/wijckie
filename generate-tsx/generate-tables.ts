...
import { stripPath, toCamel, toPascal, toSnake } from "./utils/naming";
...
const writeTable = (
  ...
  const path = `/Users/lodewijckvogelzang/git/wijckie/front-end/src/tables/modules/${toCamel(
    moduleName
  )}/${toPascal(item.name)}Table.tsx`;
...
fs.readdirSync(directory).forEach((file) => {
  ...
  writeTable(urlMap, moduleName, content.module);
  for (const widget of content.widgets) {
    writeTable(urlMap, moduleName, widget);
  }
  for (const extraClass of content.extraClasses) {
    writeTable(urlMap, moduleName, extraClass);
  }
});
